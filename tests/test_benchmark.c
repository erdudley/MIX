...
#define MIX_BENCHMARK_PATH SOURCE_DIR "/tests/data/2.mix"
#define MIX_BENCHMARK_EXPECTED_PATH SOURCE_DIR "/tests/data/2.expected"
...
MIX_TEST(test_benchmark_program_output)
{
    mix_interpreter vm;
    mix_assemble_result assembled;
    mix_run_result run_result;
    const mix_io *io;
    char *expected = mix_read_file(MIX_BENCHMARK_EXPECTED_PATH);
    ...
    assembled = mix_assemble_file(&vm, MIX_BENCHMARK_PATH);
    if (!assembled.ok) {
...
const mix_test_case mix_benchmark_tests[] = {
    {"program_output", test_benchmark_program_output},
};

const size_t mix_benchmark_test_count =
    sizeof(mix_benchmark_tests) / sizeof(mix_benchmark_tests[0]);